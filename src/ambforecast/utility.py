...
def pre_process_daily_data(df, observation_col, index_col):
    ...
    clean_table = pd.pivot_table(
        df,
        values=observation_col.lower(),
...
def get_best_arima_parameters():
    ...
    """
    params = {"order": (1, 1, 3), "seasonal_order": (1, 0, 1, 7)}
    return params
...
def multi_region_forecast(
    ...
    results = Parallel(n_jobs=-1)(
        delayed(forecast)(y_train[region], horizon) for region in regions
    )
    return results

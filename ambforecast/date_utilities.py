...
def date_ordinal(d):
    ...
    Example:
        ...
        'th'
    """
    return ('th' if 4 <= d % 100 <= 20 else {1:'st', 2:'nd', 3:'rd'}.get(d % 10, 'th'))
...
def next_monday():
    ...
    mondays = pd.date_range(today, freq='W-MON', periods=2).normalize().to_series()
...
def next_sunday():
    ...
    """
    today = pd.Timestamp.today()
    sundays = pd.date_range(today, freq='W-SUN', periods=2).normalize().to_series()

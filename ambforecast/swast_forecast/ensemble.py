...
class ProphetARIMAEnsemble():
    ...
    def __init__(self, order, seasonal_order, County, prophet_default_alpha=0.05,
                 df_holiday = None,
                 **arima_kwargs):
        '''
        ...
        df_holiday: pandas dataframe, optional (default=none)
            used for passing in existing holiday dataframe
    ...
    def _fit_arima(self, y_train):
        ...
        self.arima_model = sm.tsa.arima.ARIMA(endog=y_train,
                                 exog=arima_holidays,
                                 order=(1, 1, 3),
    ...
    def _fit_prophet(self, y_train, alpha):
        ...
        self.prophet_model = Prophet(holidays=self.holidays,
                                     ...
                                     daily_seasonality=False)#, changepoint_range=1)
    ...
    def predict(self, horizon, alpha=0.05, return_pred_int=False,
        ...
        summary_frame['yhat_upper_' + post_fix] = \
    ...
    def _prophet_predict(self, horizon, alpha):
        ...
        df = prophet_forecast[['ds', 'yhat',
                               'yhat_lower', 'yhat_upper', 'trend', 'holidays', 'weekly', 'yearly']][-horizon:]
        post_fix = str(int((1 - alpha) * 100))
        df.columns = ['ds', 'prophet_mean', 'prophet_lower_' +
                      post_fix, 'prophet_upper_' + post_fix, 'trend', 'holidays', 'weekly', 'yearly']
        df.index = df['ds']

...
class ProphetARIMAEnsemble:
    ...
    def __str__(self):
        return (
            f"ProphetARIMAEnsemble(order={self.order}, "
    ...
    def _fit_arima(self, y_train):
        ...
        arima_holidays = self._encode_holidays(
            self.holidays["ds"].to_numpy(), y_train.index
        )
    ...
    def predict(
        ...
        summary_frame["yhat_upper_" + post_fix] = summary_frame[
            ["prophet_upper_" + post_fix, "arima_upper_" + post_fix]
        ].mean(axis=1)
    ...
    def _arima_predict(self, horizon, alpha=0.05):
        ...
        df.columns = [
            "arima_mean",
            "arima_lower_" + post_fix,
            "arima_upper_" + post_fix,
        ]
        df.index.name = "ds"
        return df
    ...
    def _prophet_predict(self, horizon, alpha):
        ...
        df.columns = [
            "ds",
            "prophet_mean",
            "prophet_lower_" + post_fix,
            "prophet_upper_" + post_fix,
            "trend",
            "holidays",
            "weekly",
            "yearly",
